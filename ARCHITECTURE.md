...
public/ui.css            layer 2: boot sequence, reveals, motion, palette, skeletons
public/site.js           shared helpers: app icons, device screens, patterns, compatibility, toast, nav wiring
public/motion.js         layer 2 behaviour: boot, reveals, tilt/glare, magnetic, toasts, ⌘K palette
...
scripts/{smoke,boot-test,boot-guard-test}.js   jsdom test suites (npm test)
```
...
### Motion layer (`ui.css` + `motion.js`)

A second, additive layer on top of `styles.css` — same tokens, no framework:

- **Motion tokens.** `--ease-spring` (`cubic-bezier(.34,1.56,.64,1)`) for
  entrances and pops, `--ease-out-expo` (`cubic-bezier(.16,1,.3,1)`) for reveals
  and hovers, alongside the original HarmonyOS curve.
- **Reveals.** `[data-reveal]` (`up` / `left` / `right` / `fade` / `scale`) with
  optional `data-reveal-delay`, an `IntersectionObserver` gate, and automatic
  stagger for `[data-reveal-group]` children and card grids. `[data-split]`
  headlines wrap each word in a line mask and cascade them. Everything is
  guarded by an `html.js` class so a JS failure can never hide content — an
  inline timer strips the class if `ui-ready` never arrives.
- **Spatial boot sequence.** Home page, first visit of the session only. An
  inline head script arms `html.boot-armed` before first paint (so nothing
  flashes), a 420vh spacer drives progress from scroll position, and
  `motion.js` renders: canvas starfield with warp streaks, perspective floor,
  breathing brand mark with three expanding rings, an honest scroll-tracked
  progress bar + boot log, then 13 app tiles rushing the camera from
  `translateZ(-3000px)` to the viewer before a white flash hands the page back.
  Escape hatches: Skip button, `Esc`/`Enter`/`Space`, `prefers-reduced-motion`,
  `sessionStorage.af-booted`, `?noboot=1`, and a failsafe timer.
- **Component craft.** Skeleton loaders while the JSON API resolves, shimmer
  and sheen accents, cursor-tracked spotlight borders on cards, tilt + glare on
  the hero device, magnetic buttons with click ripples, press feedback on
  pills/tabs, count-up stats, a marquee stack band, a gliding nav pill and a
  scroll-progress hairline.
- **Command palette.** A combobox-style ⌘K overlay (`role="dialog"` +
  `role="listbox"`/`option`, `aria-activedescendant`) searching ports, pages and
  actions with subsequence scoring and `<mark>` highlighting.
- **Toast stack & sheet.** `AF.toast` is replaced with a stacked, spring-entered
  toast queue; the quick-look modal gets a spring entrance plus a drag handle
  with drag-to-dismiss physics on phones.
- **Reduced motion.** The whole layer collapses under
  `prefers-reduced-motion: reduce`: no boot, no parallax, no continuous
  animation, content rendered at rest.
...
Motion-layer specifics: the boot dialog takes focus while it is up and returns
it on exit, carries `role="dialog"`/`aria-modal` and an `aria-valuenow`
progress bar, and is skippable from the keyboard; the palette is a proper
combobox with `aria-activedescendant`; split headlines keep their original text
in `aria-label` with words `aria-hidden`; decorative layers (glow, marquee,
float chips, boot) are `aria-hidden`; and an `html.js` failsafe guarantees that
content is never hidden when scripting fails.
...
npm test                   # jsdom suites (server must be running)
```

`express` stays the only runtime dependency; `jsdom` is dev-only and exists
solely for `npm test`.
